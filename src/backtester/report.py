...
def trial_counts(cfg: Config) -> dict[str, tuple[int, float]]:
    """{"all": (N, var), "candidates": (N, var)} from the specification
    log: the row count and the variance of the monthly Sharpe across the
    rows of that kind. N is never an argument (invariant 8); the two
    counts are the deflated Sharpe's two denominators (FIX_PLAN F4)."""
    if not cfg.specifications.exists():
        return {"all": (0, 0.0), "candidates": (0, 0.0)}
    log = pl.read_csv(cfg.specifications, infer_schema_length=0)
    if "kind" not in log.columns:
        log = log.with_columns(pl.lit("").alias("kind"))
    log = log.with_columns(
        pl.when(pl.col("kind").fill_null("") != "")
        .then(pl.col("kind"))
        .otherwise(
            pl.col("note").map_elements(portfolio.spec_kind, return_dtype=pl.Utf8)
        )
        .alias("kind")
    )
    out = {}
    for name, rows in (
        ("all", log),
        ("candidates", log.filter(pl.col("kind") == "candidate")),
    ):
        s = rows["sharpe_net"].cast(pl.Float64, strict=False).drop_nulls()
        var = float((s / math.sqrt(12)).var()) if s.len() > 1 else 0.0
        out[name] = (rows.height, var)
    return out
